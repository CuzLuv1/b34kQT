...
static void udpos_ppp(rtk_t *rtk)
{
    ...
    /* initialize position for first epoch 第一个历元进行初始化*/
    if (norm(rtk->x,3)<=0.0) {
    ...
    /* static ppp mode 静态PPP*/
    if (rtk->opt.mode==PMODE_PPP_STATIC) {//X位置的状态向量在静态PPP下认为不变因此不进行更新
        for (i=0;i<3;i++) {
            rtk->P[i*(1+rtk->nx)]+=SQR(rtk->opt.prn[5])*fabs(rtk->tt);//随机游走模型更新协方差向量
        }
        return;
    }
...
static void udclk_ppp(rtk_t *rtk)
{
    ...
    /* initialize every epoch for clock (white noise) 白噪声*/
    for (i=0;i<NSYS;i++) {
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    /* handle day-boundary clock jump 周跳处理*/
    if (rtk->opt.posopt[5]) {
...
static void udstate_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    /* temporal update of position 位置更新（状态向量 协方差矩阵）*/
    udpos_ppp(rtk);
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    time2str(obs[0].time,str,2);//时间转换 通过该历元第一颗卫星的时间来确定历元
    ...
    /* temporal update of ekf states EKF状态方程更新（不依赖观测值）*/
    udstate_ppp(rtk,obs,n,nav);

    /* satellite positions and clocks 卫星位置计算（精密星历）*/
    satposs(obs[0].time,obs,n,nav,rtk->opt.sateph,rs,dts,var,svh);//广播星历 精度5m 精密星历 精度2cm svh卫星状态 

    /* exclude measurements of eclipsing satellite (block IIA) 质量较差卫星排除*/
    if (rtk->opt.posopt[3]) {
        testeclipse(obs,n,nav,rs);
    }
    /* earth tides correction 固体潮改正*/
    if (opt->tidecorr) {
