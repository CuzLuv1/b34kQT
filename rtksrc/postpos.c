...
/*判断卫星所属北斗系统*/
static int GetBDSType(uint8_t sat)
{
                    ...
                    rtk_t *rtk, int mode)
{
    ...
    while ((nobs=inputobs(obs_ptr,rtk->sol.stat,popt))>=0) {
        ...
        // }
         for (i=n=0;i<nobs;i++) {
            if(popt->navsys == SYS_CMP && popt->BDSType != 1&& popt->exsats[obs_ptr[i].sat - 1] != 1)
            {
                if ((satsys(obs_ptr[i].sat, NULL) & popt->navsys) && GetBDSType(obs_ptr[i].sat) == popt->BDSType &&
                    popt->exsats[obs_ptr[i].sat - 1] != 1)
                {
                    obs_ptr[n++] = obs_ptr[i];
                }
            }
            else if(popt->navsys == SYS_CMP && popt->BDSType == 1&& popt->exsats[obs_ptr[i].sat - 1] != 1)
            {
                if ((satsys(obs_ptr[i].sat, NULL) & popt->navsys) &&
                popt->exsats[obs_ptr[i].sat - 1] != 1) 
                {obs_ptr[n++] = obs_ptr[i];}
            ...
            ((satsys(obs_ptr[i].sat,NULL)&popt->navsys)&&
                popt->exsats[obs_ptr[i].sat-1]!=1) {obs_ptr[n++]= obs_ptr[i];}
        }
        if (n<=0) continue;
